...
//Arduino_GFX *gfx = new Arduino_ST7789(bus, LCD_RST, 5, false, 172, 320, 34, 0, 34, 0);
              ...
              //============ RGB-565 (16-bit) для моего ESP32-S3 LCD 1.47 Touch
...
void LoopDisplay() {
 ...
 char buffer[4][8]; // Создаем 4 буфера, каждый размером по 8 байт
 char clean_buffer[4][8] = {0}; // Инициализация нулями при объявлении

  //НЕБЛОКИРУЮЩИЙ ТАЙМЕР МИГАНИЯ (500 мс / 500 мс)
  bool toggleBlink = false;
  if (millis() - blinkTimer >= 500) {
    blinkTimer = millis();
    showBatteryIcon = !showBatteryIcon; // Меняем состояние (давление <-> батарейка)
    toggleBlink = true;                 // Флаг, что пора принудительно обновить экран
  }
 ...
 if ((new_FW_bar != FW_bar) || (FW_LowBat && toggleBlink)) {  // обновление данных только если данные изменились
    new_FW_bar = FW_bar;
    gfx->setFont(); 
    gfx->setTextSize(4 , 5 , 2 );
    ...
    if (FW_LowBat && showBatteryIcon) {
      gfx->setCursor(12, 13);
      gfx->print("   "); 
      gfx->fillRoundRect(12, 13, 66, 33, 4, YELLOW);
      gfx->fillRoundRect(18, 15, 58, 29, 4, BLACK);
      gfx->fillRect(77, 25, 5, 9, YELLOW);
    } else {
      gfx->setCursor(12, 13); 
      gfx->print(buffer[2]); 
    }
  ...
  */
  ...
  if ((new_RW_bar != RW_bar) || (RW_LowBat && toggleBlink)) {
    new_RW_bar = RW_bar;
    gfx->setFont(); 
    gfx->setTextSize(4, 5, 2);
    if (RW_bar <= 2.1 || RW_bar >= 2.9) {
      gfx->setTextColor(YELLOW, BLACK);
    } else {
      gfx->setTextColor(WHITE, BLACK);
    }

    if (RW_LowBat && showBatteryIcon) {
      gfx->setCursor(12, 132); 
      gfx->print("   "); 
      gfx->fillRoundRect(12, 132, 66, 33, 4, YELLOW);
      gfx->fillRoundRect(18, 134, 58, 29, 4, BLACK);
      gfx->fillRect(77, 144, 5, 9, YELLOW);
    } else {
      gfx->setCursor(12, 132); 
      gfx->print(buffer[3]); 
    }
